...
def main(date_label, input_directory, output_directory):
    ...
    file_list = glob.glob(f'{output_directory}/MOD44B.A{date_label}*.tif')
    for file_name in file_list:
        cmd = f'{cmd} {file_name}'
    os.system(cmd)
    print(f'---------------------------------------------------')
...
if __name__ == "__main__":
    # ********************** scale factor = 1.0 **********************
    input_directory = '/mnt/f/PRD_urbanization/2020_MOD44B'
    output_directory = '/mnt/f/PRD_urbanization/2020_MOD44B/PNV'
    ...
    for temp_modis_data in modis_data:
        temp_date_label = os.path.basename(temp_modis_data)[8:15]
