...
def main(date_label, input_directory, output_directory):
    ...
    for i in file_list:
        ...
        #  获取要转换的子数据集
        data = datasets.GetSubDatasets()[10][0]
    ...
    cmd = f'gdal_merge.py -o {output_directory}/mosaic/mosaic_{yyyymmdd}.tif ' \
          f'-n -9999 -ps 0.03 0.03 ' \
          f'-ul_lr {lon_min} {lat_max} {lon_max} {lat_min}'

    file_list = glob.glob(f'{output_directory}/MOD04_3K.A{date_label}*.tif')
    for file_name in file_list:
        cmd = f'{cmd} {file_name}'
    os.system(cmd)
    print(f'---------------------------------------------------')
...
if __name__ == "__main__":
    # ********************** scale factor = 0.001 **********************
    input_directory = '/mnt/d/MOD04_3K_0411/0410'
    output_directory = '/mnt/d/MOD04_3K_0411/0410/geo_band'
