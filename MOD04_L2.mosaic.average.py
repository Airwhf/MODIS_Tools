...
def main(date_label, input_directory, output_directory):
    ...
    for i in file_list:
        ...
        #  获取要转换的子数据集
        data = datasets.GetSubDatasets()[63][0]
    ...
    file_list = glob.glob(f'{output_directory}/MOD04_L2.A{date_label}*.tif')
    for file_name in file_list:
        cmd = f'{cmd} {file_name}'
    os.system(cmd)
    print(f'---------------------------------------------------')
...
if __name__ == "__main__":
    # ********************** scale factor = 0.001 **********************
    input_directory = '/mnt/d/MOD04'
    output_directory = '/mnt/d/MOD04/geo_band'
